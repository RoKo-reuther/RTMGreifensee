...
SUBROUTINE solve_tableau(               &  
    ...
    DOUBLE PRECISION, DIMENSION (N_components * N_grid)     :: component_guess     !initial guess for component concentrations
    ...
    IF (SUM(ABS(component_guess)) == 0.0) THEN
        !component_guess = abs(component_total / 1000)
        i = 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = 1e-4
        i = i + 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = component_total(((i-1) * N_grid + 1) : (i * N_grid)) * 1.828513e-01
        i = i + 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = component_total(((i-1) * N_grid + 1) : (i * N_grid)) * 6.185775e-01
        i = i + 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = component_total(((i-1) * N_grid + 1) : (i * N_grid))
        i = i + 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = component_total(((i-1) * N_grid + 1) : (i * N_grid)) * 9.944080e-01
        i = i + 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = component_total(((i-1) * N_grid + 1) : (i * N_grid)) * 9.999921e-01
        i = i + 1
        component_guess(((i-1) * N_grid + 1) : (i * N_grid)) = component_total(((i-1) * N_grid + 1) : (i * N_grid)) * 5.000000e-01
    END IF
    ...
    DO layer = 1, N_grid
        ...
        ! new try if failed before with pcfm-preconditioning - 1
        call pcfm_0D ( &
            & component_guess_ly, component_total_ly, &
            & tableau, logK,                          &
            & N_components, N_species,                &
        ...
        ! new try if failed before with pcfm-preconditioning - 2
        call pcfm_0D ( &
            & component_guess_ly, component_total_ly, &
            & tableau, logK,                          &
            & N_components, N_species,                &
        ...
        IF (all(abs(difference_ly) < atol)) success_ly = 6
